...
let state = {
  ...
  lastIRIUpdate: 0,
  initialPositionReceived: false   // Nuevo flag para controlar el centrado inicial
};
...
function updateGPSPosition(pos) {
  ...
  // Si es la primera posición, centrar el mapa inmediatamente sin condiciones
  ...
  // Para siguientes posiciones, aplicar umbrales de movimiento
  if (kmh > 3) {
...
function startGPS() {
  ...
  if ('geolocation' in navigator) {
    // Intentar una lectura rápida inicial
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        updateGPSPosition(pos);
        // Iniciar el watch normal
