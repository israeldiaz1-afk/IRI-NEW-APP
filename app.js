      ...
      afterDraw(chart) {
        ...
        items.forEach((item, i) => {
          ...
          if (item.datasetIndex === 0) {
            ...
            // IRI: línea tricolor + texto
            const segWidth = (30 - 4) / 3; // mantener línea de 30px de ancho
            // Verde
            ctx.beginPath();
            ctx.strokeStyle = '#10b981';
            ctx.lineWidth = 3;
            ctx.moveTo(box.left, y);
            ctx.lineTo(box.left + segWidth, y);
            ctx.stroke();
            // Naranja
            ctx.beginPath();
            ctx.strokeStyle = '#f59e0b';
            ctx.moveTo(box.left + segWidth + 2, y);
            ctx.lineTo(box.left + segWidth * 2 + 2, y);
            ctx.stroke();
            // Rojo
            ctx.beginPath();
            ctx.strokeStyle = '#ef4444';
            ctx.moveTo(box.left + segWidth * 2 + 4, y);
            ctx.lineTo(box.left + 30, y);
            ctx.stroke();
            // Texto con unidades
            ctx.fillStyle = '#94a3b8';
            ctx.font = '10px Inter, sans-serif';
