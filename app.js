...
let state = {
  ...
  // Estadísticas de la medición activa
  iriMax: 0,
  iriMin: Infinity,
  iriMedSum: 0,
  iriMedCount: 0
...
// ============ BASE DE DATOS DE VEHÍCULOS (sin cambios) ============
...
function processAccelerometerData(verticalAccel) {
  ...
  // --- Actualizar estadísticas de medición activa ---
  if (state.isMeasuring && !state.isPaused) {
    ...
    // Acumular para el punto de datos
...
function updateGPSPosition(pos) {
  ...
  if (kmh > 3) {
    ...
    if (dist > 1.5) {
      ...
      // Mapa de la pantalla activa
      if(state.mapMeasureActive) {
        if (!state.currentMarkerActive) {
          state.currentMarkerActive = L.marker([latitude, longitude]).addTo(state.mapMeasureActive);
        } else {
          state.currentMarkerActive.setLatLng([latitude, longitude]);
        }
  ...
  // Registrar punto de datos si estamos midiendo y no pausados
  if (state.isMeasuring && !state.isPaused && state.orientationCalibrated) {
    if (state.iriCount > 0) {
      const avgMeasured = state.iriMeasuredAccum / state.iriCount;
      const avgCorrected = state.iriCorrectedAccum / state.iriCount;
      state.currentDataPoints.push({
        timestamp: Date.now(),
        lat: latitude,
        lon: longitude,
        speed: kmh,
        iri_measured: avgMeasured,
        iri_corrected: avgCorrected
      });
